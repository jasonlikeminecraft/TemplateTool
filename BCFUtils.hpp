...
static void writeBCF(const std::string& path,  
    const std::vector<std::vector<BlockRegion>>& subChunks,  
...
{  
    ...
    // 写入状态值映射    
    header.stateValueMapOffset = ofs.tellp();    
    write_u32(ofs, stateValueMap.size());    
    for (auto& kv : stateValueMap) {    
        write_u8(ofs, kv.first);    
        writeString16(ofs, kv.second);    
    }  
...
std::map<std::string, std::string> getBlockState(int x, int y, int z) {  
...
// 获取指定方块的NBT数据  
nbt::tag_compound_ptr getBlockNBTData(int x, int y, int z) {  
    int pid = fetchPaletteId(x, y, z);  
    if (pid < 0 || pid >= static_cast<int>(palette.size())) {  
        return nullptr;  
    }  
    return palette[pid].nbtData;  
}
